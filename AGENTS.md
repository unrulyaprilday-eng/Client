...
- 默认路由：只读信息收集使用 `Terra High`；代码/文档编辑和独立审计使用 `Terra XHigh`，不得静默降级。其他任务选择能可靠完成的最低级别，不确定时上调一级。低到高：`Luna Max`（简单隔离/UI/CRUD/测试/文档）；`Terra High`（常规开发/bug/集成/中等重构）；`Terra Max`（中大型/跨模块/难 bug）；`Sol High`（重要功能/复杂业务/较大重构）；`Sol XHigh`（复杂架构/关键跨模块/困难调试）；`Sol Max`（关键架构/安全/支付钱包/数据库迁移/极端调试）。
